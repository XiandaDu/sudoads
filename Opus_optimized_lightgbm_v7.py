# optimized_lightgbm_v7.py
# Enhanced version targeting 0.42+ based on V6 insights
...
class EnhancedModelV7:
    """Model V7: Targeting 0.42+ by enhancing top-performing features"""
    ...
    def create_ultimate_features(self, df_vwap, df_high, df_low, df_amount, df_open, df_close, df_volume):
        """Enhanced features based on V6 importance analysis"""
        features = {}
        
        # === TOP PRIORITY: VOLATILITY FEATURES (highest importance) ===
        returns_15min = (df_vwap / df_vwap.shift(1) - 1).astype(self.dtype)
        
        # 1. Multiple volatility windows (336 was #1 feature!)
        for window in [48, 96, 168, 336, 672]:  # More granular windows
            ...
            # NEW: Volatility change rate
            vol_change = vol / vol.shift(window//4) - 1
        ...
        vol_672 = returns_15min.rolling(672, min_periods=336).std()
        
        features['volatility_ratio_96_336'] = (vol_96 / (vol_336 + 1e-8)).astype(self.dtype)
        features['volatility_ratio_336_672'] = (vol_336 / (vol_672 + 1e-8)).astype(self.dtype)
        
        # Volatility percentile (multiple lookbacks)
        for window in [96, 336]:
            vol = returns_15min.rolling(window, min_periods=window//2).std()
            for lookback in [window*2, window*4, window*8]:
                vol_percentile = vol.rolling(lookback, min_periods=lookback//2).rank(pct=True)
                features[f'volatility_percentile_{window}_{lookback}'] = vol_percentile.astype(self.dtype)
        
        # === HIGH-LOW SPREAD (3rd most important) ===
        ...
        for window in [48, 96, 168, 336]:
            ...
            # NEW: Spread expansion/contraction
            spread_change = hl_spread.rolling(window).mean() / hl_spread.rolling(window*2).mean() - 1
            features[f'spread_change_{window}'] = spread_change.astype(self.dtype)
        
        # === EFFICIENCY RATIO (4th most important) ===
        for window in [48, 96, 168, 336]:
        ...
        # === RELATIVE MOMENTUM (6th most important) ===
        for window in [24, 96, 336]:
            ...
            # NEW: Momentum vs market percentile
            mom_percentile = returns.rank(axis=1, pct=True)
            features[f'momentum_percentile_{window}'] = mom_percentile.astype(self.dtype)
        
        # === RSI DIVERGENCE (7th most important) ===
        for period in [48, 96, 168]:
            delta = df_vwap.diff()
        ...
        # === RETURNS (8th most important, especially 672) ===
        for window in [1, 4, 24, 96, 168, 336, 672, 1344]:  # Extended windows
            ...
            # NEW: Return consistency
            if window > 24:
                ret_sign = (ret > 0).astype(self.dtype)
                consistency = ret_sign.rolling(window//4, min_periods=window//8).mean()
        ...
        for window in [96, 336, 672]:
            ...
            # NEW: Volume profile
            vol_profile = df_amount.rolling(window).sum() / df_amount.rolling(window*2).sum()
            features[f'volume_profile_{window}'] = vol_profile.astype(self.dtype)
        
        # === MICROSTRUCTURE ENHANCEMENTS ===
        overnight_gap = (df_open - df_close.shift(1)) / (df_close.shift(1) + 1e-8)
        for window in [48, 96, 168]:
            features[f'overnight_gap_mean_{window}'] = overnight_gap.rolling(
                window, min_periods=window//2
            ).mean().astype(self.dtype)
            features[f'overnight_gap_std_{window}'] = overnight_gap.rolling(
                window, min_periods=window//2
        ...
        # NEW: Parkinson volatility (using high-low)
        for window in [96, 336]:
            parkinson = np.sqrt(
        ...
        # NEW: Yang-Zhang volatility (more accurate)
        for window in [96, 336]:
            # Overnight
    ...
    def train_advanced_ensemble(self, X_train_scaled, y_train, feature_names):
        """Advanced ensemble with more diversity"""
        ...
        # Model 1: Best parameters (seed 42)
        print("Model 1/6: Best params (seed=42)")
        params1 = base_params.copy()
        params1.update({'seed': 42, 'bagging_seed': 42, 'feature_fraction_seed': 42})
        
        # Create separate dataset for each model to avoid feature pre-filter issues
        ...
        print("Model 2/6: Best params (seed=123)")
        ...
        print("Model 3/6: Lower LR, more trees")
        ...
        # Model 6: Slightly different tree structure
        print("Model 6/6: Modified tree structure")
        params6 = base_params.copy()
        params6['num_leaves'] = 180
        params6['min_data_in_leaf'] = 120
        params6['max_depth'] = 12
        params6.update({'seed': 7777, 'feature_pre_filter': False})  # Disable pre-filter for this model
    ...
    def advanced_post_processing(self, predictions, percentile_low=0.25, percentile_high=99.75):
        """Advanced post-processing with adaptive clipping"""
        
        # 1. Adaptive clipping based on distribution
        predictions = np.clip(predictions, 
                            np.percentile(predictions, percentile_low), 
                            np.percentile(predictions, percentile_high))
        
        # 2. Outlier smoothing using Winsorization
        from scipy.stats import mstats
        predictions = mstats.winsorize(predictions, limits=(0.002, 0.002))
        
        # 3. Normalize to realistic return distribution
        mean_pred = np.mean(predictions)
        std_pred = np.std(predictions)
        if std_pred > 0:
            # Target volatility of 2% (typical for crypto 24h returns)
            predictions = (predictions - mean_pred) / std_pred * 0.02
            
            # 4. Apply slight mean reversion to extreme predictions
            extreme_threshold = 3  # 3 standard deviations
            extreme_mask = np.abs(predictions) > (0.02 * extreme_threshold)
            predictions[extreme_mask] *= 0.85  # Reduce extreme predictions by 15%
    ...
    def train_efficient(self, df_target, df_factor1, df_factor2, df_factor3, 
        ...
        print("Starting enhanced training for V7...")
        ...
        # Use 35% for training (slightly more than V6)
        ...
        self.train_advanced_ensemble(X_train_scaled, y_train, feature_names)
    ...
    def train(self, df_target, df_factor1, df_factor2, df_factor3):
        ...
        # Calculate volume from amount and vwap
        ...
        additional_features = self.create_ultimate_features(
        ...
        if hasattr(self, 'models') and len(self.models) > 0:
            ...
            with open('./result/model_v7_info.json', 'w') as f:
    ...
    def _prepare_submission(self, results_df):
        ...
        df_submit_competition.to_csv("./result/submit_v7.csv", index=False)
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("ENHANCED MODEL V7 - TARGETING 0.42+ WEIGHTED SPEARMAN")
    print("="*70)
    print("\nKey improvements over V6 (0.4091):")
    print("1. Expanded volatility features (top importance in V6)")
    print("2. Enhanced high-low spread calculations")
    print("3. Advanced volatility measures (Parkinson, Yang-Zhang)")
    print("4. More granular efficiency ratios")
    print("5. Extended return windows (up to 1344 periods)")
    print("6. 6-model ensemble with diverse objectives")
    print("7. Advanced post-processing with adaptive clipping")
    print("8. 35% training data (vs 30% in V6)")
    print("\n" + "="*70 + "\n")
    
    model = EnhancedModelV7()
    model.run()