# optimized_lightgbm_v10_lite.py
# Memory-efficient version focusing on proven winners
...
class EfficientFinalModel:
    """V10 Lite: Memory-efficient with focus on proven winners"""
    ...
    def create_optimized_features(self, df_vwap, df_high, df_low, df_amount, df_open, df_close):
        """Optimized features based on V9 winners"""
        features = {}
        
        # === TOP PRIORITY 1: DISTANCE FROM LOW (especially 96) ===
        # Focus on key windows that performed best
        for window in [84, 96, 108, 120, 168, 240, 336, 480, 672]:
            high_roll = df_high.rolling(window, min_periods=window//2).max()
            low_roll = df_low.rolling(window, min_periods=window//2).min()
            
            # Distance from low (top feature!)
            dist_from_low = ((df_vwap - low_roll) / df_vwap).astype(self.dtype)
            features[f'distance_from_low_{window}'] = dist_from_low
            
            # Special variations for window 96
            if window == 96:
        ...
        # === TOP PRIORITY 2: RSI DIVERGENCE (especially 48) ===
        ...
        # Special RSI_48 variations (since it was #2 in V9)
        ...
        # === TREND STRENGTH (showed importance in V9) ===
        for window in [672]:
    ...
    def train_efficient_ensemble(self, X_train_scaled, y_train, feature_names):
        """Efficient ensemble with 6 models"""
        print("Training efficient ensemble...")
        ...
        print("Model 2/6: Different seed")
        ...
        # Model 6: More leaves
        print("Model 6/6: More leaves")
        params6 = base_params.copy()
        params6.update({'num_leaves': 200, 'min_data_in_leaf': 100, 'seed': 7777, 'feature_pre_filter': False})
    ...
    def post_process(self, predictions):
        ...
        predictions = np.clip(predictions, 
                            np.percentile(predictions, 0.2), 
                            np.percentile(predictions, 99.8))
        
        # Winsorize
        from scipy.stats import mstats
        predictions = mstats.winsorize(predictions, limits=(0.0018, 0.0018))
        
        # Normalize
        mean_pred = np.mean(predictions)
        std_pred = np.std(predictions)
        if std_pred > 0:
            predictions = (predictions - mean_pred) / std_pred * 0.0218
            
            # Smooth extremes
            extreme_mask = np.abs(predictions) > (0.0218 * 3.1)
            predictions[extreme_mask] *= 0.86
    ...
    def train_efficient(self, df_target, df_factor1, df_factor2, df_factor3, 
        ...
        # Use 38% for training (slightly less than 40% for memory)
        print("Preparing training data (38% of symbols)...")
        train_symbols = all_symbol_list[:int(len(all_symbol_list) * 0.38)]
        
        # Prepare data in chunks to save memory
        train_features = []
        feature_names = []
        ...
        # Use RobustScaler (less memory than QuantileTransformer)
        ...
        # Train ensemble
        self.train_efficient_ensemble(X_train_scaled, y_train, feature_names)
        
        self.feature_names = feature_names
        self.scaler = scaler
        
        # Free memory before predictions
        del train_features, train_df, X_train, X_train_scaled
        gc.collect()
        
        # Process all data in small chunks
        print("Making predictions...")
        chunk_size = 12  # Smaller chunks for memory
        results_list = []
    ...
    def train(self, df_target, df_factor1, df_factor2, df_factor3):
        ...
        additional_features = self.create_optimized_features(
        ...
        if hasattr(self, 'models') and len(self.models) > 0:
            ...
            with open('./result/model_v10_lite_info.json', 'w') as f:
    ...
    def _prepare_submission(self, results_df):
        ...
        df_submit_competition.to_csv("./result/submit_v10_lite.csv", index=False)
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("EFFICIENT V10 LITE - MEMORY-OPTIMIZED FINAL PUSH")
    print("="*70)
    print("\nKey optimizations:")
    print("1. Focus on distance_from_low (especially 96)")
    print("2. Comprehensive RSI divergence features")
    print("3. 38% training data (memory-efficient)")
    print("4. RobustScaler (less memory than QuantileTransformer)")
    print("5. 6-model ensemble")
    print("6. Smaller processing chunks (12 symbols)")
    print("7. Aggressive memory cleanup")
    print("8. ~90 features total")
    print("\n" + "="*70 + "\n")
    
    model = EfficientFinalModel()
    model.run()