# optimized_lightgbm_v8_lite.py
# Memory-efficient version targeting 0.57+ based on V7's success (0.5549)
...
class EfficientModelV8:
    """Model V8 Lite: Memory-efficient version focused on proven winners"""
    ...
    def create_essential_features(self, df_vwap, df_high, df_low, df_amount, df_open, df_close):
        """Only the most essential features based on V7's top performers"""
        features = {}
        
        # === TOP PRIORITY: RSI DIVERGENCE (Top feature!) ===
        # Focus on the most important RSI periods
        for period in [36, 48, 60, 72, 96]:  # Reduced from 8 to 5 periods
            delta = df_vwap.diff()
            ...
            # RSI momentum
            if period == 48:  # Only for the best period
                features[f'rsi_momentum_{period}'] = (rsi - rsi.shift(period//2)).astype(self.dtype)
        
        # === CRITICAL: DISTANCE FROM LOW (2nd, 4th, 5th features!) ===
        # Focus on most important windows
        for window in [96, 168, 240, 336, 480, 672]:  # Reduced from 10 to 6
            ...
            # Distance from high (important but less critical)
            if window in [168, 336, 672]:
        ...
        # === HIGH PRIORITY: HL SPREAD STD (3rd, 6th features) ===
        hl_spread = (df_high - df_low) / df_vwap
        
        for window in [48, 96, 168, 336]:  # Key windows only
            # Spread mean
        ...
        for window in [96, 168, 336, 672]:  # Key windows
        ...
        # Volatility percentiles (important in V7)
        for window in [96, 336]:
            vol = returns_15min.rolling(window, min_periods=window//2).std()
            vol_percentile = vol.rolling(window*8, min_periods=window*4).rank(pct=True)
            features[f'volatility_percentile_{window}_{window*8}'] = vol_percentile.astype(self.dtype)
        
        # === RETURN CONSISTENCY (good in V7) ===
        for window in [672, 1344]:  # Long windows were important
        ...
        features[f'overnight_gap_mean_96'] = overnight_gap.rolling(
            96, min_periods=48
        ).mean().astype(self.dtype)
        
        # === Yang-Zhang volatility (was valuable) ===
        for window in [96, 336]:
        ...
        # === Core returns ===
        for window in [1, 4, 24, 96, 168, 336]:
            ret = (df_vwap / df_vwap.shift(window) - 1).astype(self.dtype)
            features[f'return_{window}'] = ret
            features[f'return_{window}_rank'] = ret.rank(axis=1, pct=True).astype(self.dtype)
    ...
    def train_focused_ensemble(self, X_train_scaled, y_train, feature_names):
        """Focused ensemble with 6 diverse models"""
        print("Training focused ensemble...")
        ...
        # Model 6: Different tree structure
        print("Model 6/6: Modified structure")
    ...
    def refined_post_processing(self, predictions, percentile_low=0.2, percentile_high=99.8):
        ...
        if std_pred > 0:
            predictions = (predictions - mean_pred) / std_pred * 0.021
            
            # 4. Smooth extremes
            extreme_threshold = 3.5
            extreme_mask = np.abs(predictions) > (0.021 * extreme_threshold)
    ...
    def train_efficient(self, df_target, df_factor1, df_factor2, df_factor3, 
        ...
        print("Starting efficient training for V8 Lite...")
        ...
        # Use 35% for training (same as V7 which worked well)
        ...
        # Train ensemble
        self.train_focused_ensemble(X_train_scaled, y_train, feature_names)
        
        self.feature_names = feature_names
        self.scaler = scaler
        
        # Process all data in smaller chunks for memory efficiency
        ...
        for i in range(0, len(all_symbol_list), chunk_size):
            ...
            del chunk_features, chunk_df, X_chunk, X_chunk_scaled, predictions
            gc.collect()
    ...
    def train(self, df_target, df_factor1, df_factor2, df_factor3):
        ...
        additional_features = self.create_essential_features(
        ...
        if hasattr(self, 'models') and len(self.models) > 0:
            ...
            with open('./result/model_v8_lite_info.json', 'w') as f:
    ...
    def _prepare_submission(self, results_df):
        ...
        df_submit_competition.to_csv("./result/submit_v8_lite.csv", index=False)
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("EFFICIENT MODEL V8 LITE - MEMORY-OPTIMIZED 0.57+ TARGET")
    print("="*70)
    print("\nKey features (memory-efficient version):")
    print("1. Focused RSI divergence features (5 periods)")
    print("2. Essential distance from low/high (6 windows)")
    print("3. Critical HL spread std features")
    print("4. ~80 total features (vs 100+ in full V8)")
    print("5. 6-model ensemble")
    print("6. 35% training data (same as V7)")
    print("7. Smaller processing chunks (15 vs 20)")
    print("8. Aggressive memory cleanup")
    print("\n" + "="*70 + "\n")
    
    model = EfficientModelV8()
    model.run()