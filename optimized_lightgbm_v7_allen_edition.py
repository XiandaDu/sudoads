...
class GPUEnhancedModelV7:
    ...
    def get_all_symbol_kline(self):
        ...
        volume_arr = (
            pd.concat([i.get()["volume"] for i in df_list], axis=1)
            .sort_index(ascending=True)
            .values.astype(self.dtype)
        )
    ...
    def create_advanced_features(
        self, df_vwap, df_high, df_low, df_amount, df_open, df_close, df_volume
    ):
        """构建高级特征（与原版一致）"""
        features = {}
        ...
        features["volatility_ratio_24_96"] = (vol_24 / (vol_96 + 1e-8)).astype(
            self.dtype
        )
        features["volatility_ratio_96_336"] = (vol_96 / (vol_336 + 1e-8)).astype(
            self.dtype
        )

        for window in [96, 336]:
            vol = returns_15min.rolling(window, min_periods=window // 2).std()
            vol_percentile = vol.rolling(window * 4, min_periods=window * 2).rank(
                pct=True
            )
            features[f"volatility_percentile_{window}"] = vol_percentile.astype(
                self.dtype
            )

        # 2) 微结构
        for window in [24, 96]:
            illiquidity = (returns_15min.abs() / (df_volume * df_vwap + 1)).rolling(
                window
            ).mean() * 1e9
        ...
        for window in [24, 96]:
            features[f"order_imbalance_{window}"] = (
                order_imbalance.rolling(window).mean().astype(self.dtype)
            )
            features[f"order_imbalance_std_{window}"] = (
                order_imbalance.rolling(window).std().astype(self.dtype)
            )
        ...
        for window in [96, 336]:
            ...
            weighted_ret = df_vwap.rolling(window).mean()  # 简化实现
        ...
        for window in [24, 96, 336]:
            ...
            features[f"volume_usd_{window}_rank"] = vol_mean.rank(
                axis=1, pct=True
            ).astype(self.dtype)
            vol_std = volume_usd.rolling(window).std()
            features[f"volume_volatility_{window}"] = (
                vol_std / (vol_mean + 1e-8)
            ).astype(self.dtype)
        ...
        for window in [96, 336]:
            positive_periods = (
                (returns_15min > 0).rolling(window, min_periods=window // 2).sum()
            )
        ...
        features["overnight_gap_mean_96"] = (
            overnight_gap.rolling(96, min_periods=48).mean().astype(self.dtype)
        )
        hl_spread = (df_high - df_low) / df_vwap
        features["hl_spread_96"] = (
            hl_spread.rolling(96, min_periods=48).mean().astype(self.dtype)
        )
    ...
    def train_neural_ranker(self, X_train, y_train, X_val, y_val):
        ...
        train_loader = DataLoader(
            TensorDataset(X_train_tensor, y_train_tensor), batch_size=4096, shuffle=True
        )
        val_loader = DataLoader(
            TensorDataset(X_val_tensor, y_val_tensor), batch_size=4096, shuffle=False
        )

        input_dim = X_train.shape[1]
        self.nn_model = NeuralNetworkRanker(
            input_dim, hidden_sizes=[1024, 512, 256, 128], dropout=0.3
        ).to(device)
    ...
    def train_mega_ensemble(self, X_train_scaled, y_train, feature_names):
        ...
        try:
            ...
            _ = lgb.train(test_params, test_train, num_boost_round=1)
            use_gpu = True
        ...
        model_configs = [
            ...
            (
                dict(base_params, objective="huber", alpha=0.9, metric="mae"),
                300,
                0.10,
                "Huber loss",
            ),
            (
                dict(base_params, objective="regression_l1", metric="mae"),
                300,
                0.05,
                "MAE loss",
            ),
            (
                dict(base_params, num_leaves=320, max_depth=14),
                250,
                0.10,
                "Deeper trees",
            ),
            (
                dict(base_params, num_leaves=128, min_data_in_leaf=200),
                350,
                0.05,
                "Simpler trees",
            ),
            (
                dict(base_params, lambda_l1=1.0, lambda_l2=0.5),
                300,
                0.05,
    ...
    def train_efficient(
        ...
        # -------- 构造训练集合并：统一 MultiIndex 名称，并对齐到 target --------
        train_features = []
        feature_names = []
        ...
        train_target.index = train_target.index.set_names(["datetime", "symbol"])

        for feat_name, feat_data in all_features.items():
            if isinstance(feat_data, pd.DataFrame):
                s = feat_data[train_symbols].stack()
                s.name = feat_name
                s.index = s.index.set_names(["datetime", "symbol"])
                # 显式对齐到目标索引（防止任何时间戳或符号差异）
                s = s.reindex(train_target.index)
                train_features.append(s)
                feature_names.append(feat_name)

        # 使用 join='inner' 再做一道保险
        train_df = pd.concat(train_features + [train_target], axis=1, join="inner")
        train_df = train_df.dropna()

        # 简单探针：若后续仍有问题，可打开打印
        # base_idx = train_target.index
        # for s in train_features:
        #     if not s.index.equals(base_idx):
        #         print("Index mismatch in feature:", s.name)
        ...
        for i in range(0, len(all_symbol_list), chunk_size):
            ...
            print(
                f"Processing chunk {i//chunk_size + 1}/{(len(all_symbol_list) + chunk_size - 1)//chunk_size}..."
            )
            ...
            chunk_target.index = chunk_target.index.set_names(["datetime", "symbol"])

            for feat_name in feature_names:
                df_feat = all_features[feat_name]
                s = df_feat[chunk_symbols].stack()
                s.name = feat_name
                s.index = s.index.set_names(["datetime", "symbol"])
                s = s.reindex(chunk_target.index)  # 对齐到 chunk 目标
                chunk_features.append(s)

            chunk_df = pd.concat(
                chunk_features + [chunk_target], axis=1, join="inner"
            ).dropna()
            if len(chunk_df) == 0:
                continue
    ...
    def train(
        ...
        """主训练函数：使用 run() 里已加载的数据，避免二次加载造成索引漂移"""
        ...
        rho_overall = self.weighted_spearmanr(
            results_df["target"], results_df["y_pred"]
        )
            ...
            importance = pd.DataFrame(
                {
                    ...
                    "importance": self.models[0].feature_importance(
    ...
    def _prepare_submission(self, results_df):
        ...
        if "level_0" in df_submit.columns and "level_1" in df_submit.columns:
            df_submit.rename(
                columns={"level_0": "datetime", "level_1": "symbol"}, inplace=True
            )
        ...
        if missing_elements:
            ...
            df_submit_competition = pd.concat(
                [df_submit_competition, new_rows], ignore_index=True
            )
    ...
    def run(self):
        ...
        df_vwap = pd.DataFrame(
            vwap_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_open = pd.DataFrame(
            open_price_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_high = pd.DataFrame(
            high_price_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_low = pd.DataFrame(
            low_price_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_close = pd.DataFrame(
            close_price_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_amount = pd.DataFrame(
            amount_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )
        df_volume = pd.DataFrame(
            volume_arr, columns=all_symbol_list, index=time_arr, dtype=self.dtype
        )

        # 计算因子（目标与基础因子均使用同一套索引/符号）
        windows_1d = 4 * 24 * 1
        windows_7d = 4 * 24 * 7
        ...
        df_7d_volatility = (
            df_15min_rtn.rolling(windows_7d, min_periods=windows_7d // 2)
            .std(ddof=1)
            .astype(self.dtype)
        )
        df_7d_momentum = (df_vwap / df_vwap.shift(windows_7d) - 1).astype(self.dtype)
        df_amount_sum = (
            df_amount.rolling(windows_7d, min_periods=windows_7d // 2)
            .sum()
            .astype(self.dtype)
        )
        ...
        # —— 直接把 DataFrame 和符号列表传给训练，不再二次加载 ——
        self.train(
            df_24hour_rtn.shift(-windows_1d),  # target
            df_7d_volatility,
            df_7d_momentum,
            df_amount_sum,
            df_vwap,
